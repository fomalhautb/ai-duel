...
- `SkillCard`（`kind: 'skill'`）打出后直接进弃牌堆。多数只有卡面和亮相动画；
  卡面标了 `target` 的要在打出时指定目标，现在只有一档 `'foe-agent'`
...
弃牌堆。`AgentInstance` 除了身份三件套（`instanceId` / `cardId` / `owner`）只多一个
...
眼下卡池只有 6 张、示例牌组把它们全用上了，`INITIAL_COLLECTION` 因此暂时等于整个卡池，
...
同排其余的压暗，整排不吃指针事件（指针八成还压在它身上，不掐掉它会一直 hover 放大挡住战场）。
`castingId` 这张牌是"已打出待受理"记录（`playedRef`）里的例外：那些牌停在落点上不参与排布，
它却要照常排回扇形。
...
- **强制展示**（对手出牌，不可打断）：收到对手的 `AGENT_DEPLOYED` / `SKILL_PLAYED`，
  ...
  之后 AI 卡从展示位接着飞到对方战场行并播上场特效；技能牌要么原地淡出（无目标），
...
- **对局界面已经是真 UI**：底部 `HandFan` 扇形手牌拖进战场（或原地点一下）出牌、
  ...
  选目标态（两条路合法目标都亮橙圈），命中时技能卡飞到目标格上闪一下，
  那个 AI 从此挂着「已干扰」角标。
...
2. **技能牌的效果**——「必须回答」已经有完整的选目标交互和命中动画，
   ...
   在拼 Prompt 那一步生效。
3. **补卡池和题库**——卡池只有 4 张 AI 卡 + 2 张技能牌，题库 5 道占位题
