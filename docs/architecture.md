...
`ROUND_SCORED` 除了 `gains` / `scores`，还带着判定的**全部依据**：`correct`（双方本轮
算不算答对，团队口径）、`spent`（双方本轮消耗）、`verdict`（`'sole-correct'` /
`'fewer-tokens'` / `'equal-tokens'`，见 3.4 的三条判定）。带全是有原因的：
`correct` 光看 `AI_ANSWERED` 推不出来——场上没有 AI 的一方一条事件都没有，却也算没答对。
...
有一方单独拿到 `WIN_TARGET`（3）分、或者题库出完，`phase` 就变 `'finished'`。
...
- `questions` 是本局洗好的题序，`questions[round - 1]` 是本轮的题。
  每道题带 `keywords`（出牌阶段就公开，界面挂在「下一题」牌匾下面）。
...
1. **只有一方答对** → 该方 +1。「己方答对」是团队口径：己方场上**至少一个** AI 答对即算，
   场上没有 AI 的一方视为没答对。个体答错照常罚下，但不直接影响这个判定。
2. **双方结果相同**（都对或都错）→ 比 `PlayerState.spentThisRound`（本轮为新打出的牌
   花掉的 Token），严格少的一方 +1。老 AI 留场不重复计费，所以"什么都不打"是消耗 0 的合法打法。
3. **消耗也相同** → 双方各 +1。正因为有这一档，才会出现"双方同时到 3 分"。

结算后：一方分数 ≥ 3 且双方分数不相等就结束（题库还剩题也不打了）；
同时到线且分数相同则继续加赛；题库出完仍未分出时保底比总分，相同才是 `'draw'`。
判定的全部依据都在 `ROUND_SCORED` 事件里（见 3.2），客户端不该自己再算一遍。
...
开局各 `INITIAL_TOKEN_MAX`（5 点），每轮答题结算后 `tokenMax += TOKEN_MAX_GROWTH`（1 点）
并把 `tokens` 补满——**省下来的不跨轮累积**，直接被新的满额盖掉。
涨得慢是有意的：一局最短三轮就结束，涨太快的话最后一轮想买什么买什么，
「同结果时比本轮消耗」那条决胜线就没有分量了。
`playCard` 里费用那道闸排在选目标之前：不然客户端会让玩家挑完目标才被告知打不起。
`HeroCard` 没有这个字段，它不进牌组也就不用出。

另有一份 `spentThisRound` 记这一轮**花出去了多少**，每轮推进时清零，它是 Token 决胜的依据。
不拿 `tokenMax - tokens` 现算：那个差值眼下数值相同，但表达的是"额度剩多少"，
一旦出现「回复 Token」或「本轮额度临时 +N」的效果就不再等于消耗。
...
弃牌堆。`AiInstance` 除了身份三件套（`instanceId` / `cardId` / `owner`）只多一个
`interfered`：被干扰类技能命中过。它眼下**只挡住"再被干扰一次"并让战场小卡挂个「已干扰」角标**，
不影响答题——真往 AI 上下文里塞话的效果还没做。
...
教学局天然可预测。为它准备的口子已经在 core 里就位（见 3.4 末尾那张表）：
...
不走 `script.ts` 那张正式剧本表。对手则复用 `localDriver` 包一层按剧本出牌。
...
- **发牌**（`CARD_DRAWN`）：新牌从侧栏那摞卡堆飞进自己的扇形槽位，两排手牌各有一份实现
  ...
  开局那 5 张等抛硬币演完，每轮结算后补的牌等答题揭晓层淡出——`ROUND_SCORED` 和
  `CARD_DRAWN` 是同一批事件，收到时揭晓层（1100）才刚开始演，不拦的话牌就在遮罩后面飞完了。
  ...
  **开局那条兜底到点时还要再看一眼"硬币是不是正演着"**（`coinUpRef`），
  ...
  `ROUND_SCORED` 后面还跟不跟着 `CARD_DRAWN`：最后一轮后面跟的是 `GAME_OVER`、没有补牌，
  拦了就再没人来放行；测试面板的加牌只有 `CARD_DRAWN`、没有遮罩要等，该当场就飞。
...
- **答题制的对局流程整条打通了**：抛硬币定先手 → 双方轮流出牌（每轮至多一张新 AI 牌，
  ...
  同结果比本轮 Token 消耗，消耗也相同各 +1）→ 先到 3 分获胜，同时到分继续加赛，
  题库出完保底比总分（含平局）。判定的全部依据都在 `ROUND_SCORED` 里（见 3.2 / 3.4）。
...
- **对局界面已经是真 UI**：底部 `HandFan` 扇形手牌拖进战场（或原地点一下）出牌、
  ...
  右侧栏是一块「下一题：<类别>」的繁复牌匾、匾下挂一排题目关键词标签，
  再加一排 Token 四芒星（亮的是还剩的、灰的是花掉的，上限超过 8 点自动折成两列），
  比分在顶栏；答题揭晓层的结算除了比分还有一句判定理由（谁独对 / 比 Token 时双方各花了多少）；
...
4. **补卡池和题库**——卡池是 18 张 AI 牌 + 2 张技能牌，题库 5 道占位题
   （正常一局三五轮就分出胜负，5 道是加赛的上限），视觉测试那两道的图还没有，
