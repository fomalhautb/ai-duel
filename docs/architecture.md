...
- **阿达·洛芙莱斯 · 第一算法**——整局每一轮的 Token 上限额外 +2。
  只在 `createGame` 的 `makePlayer` 里把起点抬高 `ADA_TOKEN_MAX_BONUS`（开局 6/6），
  之后 `confirmRound` 走的是 `tokenMax += TOKEN_MAX_GROWTH` 的增量逻辑，
  这个 +2 会自然贯穿整局（第 2 轮 8、第 3 轮 10……恒比对手多 2）。
...
不记 `roundTokenSpent`、也不结束出牌轮，发动完照样接着出牌。
...
被干扰过的单位照样能升降级，两个标记互不影响。

`Card` 是卡牌**定义**（同一张卡在牌组里可以有多份），`CardInstance` / `AiInstance` 是
**实例**，带独立的 `instanceId`。AI 牌上场时沿用手牌那一份 `instanceId`，罚下时才能原样塞回
弃牌堆。`AiInstance` 除了身份三件套（`instanceId` / `cardId` / `owner`）只多两个可选字段：
`interfered` 是被干扰类技能命中过，眼下**只挡住"再被干扰一次"并让战场小卡挂个「已干扰」角标**，
不影响答题——真往 AI 上下文里塞话的效果还没做；
`levelShift` 是净升降级次数（升 +1 降 -1），**纯粹给界面画角标用**，引擎不读它。
...
- **对局界面已经是真 UI**：底部 `HandFan` 扇形手牌拖进战场（或原地点一下）出牌、
  ...
  战场右上吊着一块「下一题：<类别>」的繁复牌匾、最右缘一条 Token 细条竖着排四芒星
  （亮的是还剩的、灰的是花掉的，永远单列，点数多了只压间距），比分在顶栏；
  对手出的牌会被强制展示在屏幕中央（见 5.8），点战场小卡也能放大查看；
  要选目标的技能牌拖出去松手在谁身上就打谁、点一下则进全屏压暗的
  选目标态（两条路合法目标都亮橙圈），命中时技能牌飞到目标格上闪一下，
  那个 AI 从此挂着「已干扰」角标；主动英雄技能复用同一套选目标态，
  换过卡的单位挂「已升级 / 已降级」角标（见 5.5）。
