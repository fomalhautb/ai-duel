...
```
ROUND_STARTED → PLAY_TURN_STARTED → AGENT_DEPLOYED → SKILL_PLAYED → SKILL_CANCELED
  → PLAY_TURN_STARTED → QUESTION_REVEALED → AGENT_ANSWERED × N → AGENT_ELIMINATED
  → ROUND_SCORED → GAME_OVER
```

`SKILL_CANCELED` 只在英雄技能抵消了那张技能卡时才有，紧跟在它的 `SKILL_PLAYED` 后面
...
**卡牌和单位。** 全系统只有三种卡，靠 `kind` 区分，其中前两种进牌组：
...
- `HeroCard`（`kind: 'hero'`）**不进牌组**：它不在 `Card` 联合里，也不在 `CARDS` / `CARD_POOL` /
  `STARTER_DECK` 里，单独一张表 `src/heroes.ts`（`HEROES` + `getHero`）。
  英雄技能不占 20 张牌的牌组空间（见游戏机制文档第 4 节），塞进卡池还会连累存档过滤、
  抽卡和洗牌——那几处都是"遍历卡池"的写法，多出一张抽不到也打不出的卡只会变成脏数据。

进牌组的两类卡都**没有费用字段**：出牌不花任何资源，一轮里想打几张打几张。

**英雄。** `PlayerState.hero`（`HeroId | null`）是这一方选的英雄，
...
眼下只有一位英雄：格蕾丝·霍珀，技能 **Debug——每局抵消对方打出的第一张技能卡的效果**。
判定就在 `playCard` 的技能卡分支里：牌照常进弃牌堆、照常发 `SKILL_PLAYED`，
之后如果对手 `hero === 'grace-hopper'` 且技能没用过，就把标志置上并追发一条 `SKILL_CANCELED`
（顺序不能反，客户端要先演牌再演抵消）。技能卡目前本来就没有效果，所以这里没什么可拦的；
**将来给技能卡实现效果时，效果必须写在"没被抵消"的那条分支里**，否则 Debug 只剩一层动画。
...
- **答题全屏揭晓**（`QUESTION_REVEALED` → `AGENT_ANSWERED` × N → `ROUND_SCORED`）：
  ...
  **答题结果渲染在这一层内部，不去动战场上那张小卡**：事件是在 React 提交新快照**之前**
  ...
  不回头读快照（快照里那个单位已经没了）。
- **技能卡亮相**（`SKILL_PLAYED`）：双方都在战场中央把卡面亮一下再进弃牌堆。
  技能卡不上场，不亮出来的话画面上根本看不出有人打过牌。
- **英雄技能抵消**（`SKILL_CANCELED`）：和抛硬币、答题揭晓同一档的全屏过场（`.skill-cancel`，
  ...
  **它必须等技能卡亮相演完才上**：`SKILL_CANCELED` 和它对应的 `SKILL_PLAYED` 是同一批事件，
  ...
  连打两张技能卡时上一张的时间线还在跑，布尔会被第一条收尾提前清掉。
...
`kind`（`'agent' | 'skill' | 'hero'`）、`name`、`text`、AI 卡才有的 `model`，
...
对局左侧栏每位玩家名字旁边那张小英雄卡（`.battle__hero`，按 `--hero-card-scale` 整体缩小，
和战场小卡一个路子），以及图鉴页的英雄卡分组。它的卡面数据只有一份拼法，
...
- **强制展示**（对手出牌，不可打断）：收到对手的 `AGENT_DEPLOYED` / `SKILL_PLAYED`，
  ...
  之后 AI 卡从展示位接着飞到对方战场行并播上场特效，技能卡没有落点，原地淡出。
...
技能卡在战场中央淡入亮相（`.battle__skill-show`）——自己打的什么牌不需要被强制看一遍。
...
- **受理不了就降级，不排队**：展示层被占着（对手连着出牌）或找不到起飞点时，
  对方的 AI 卡退回一段简易的放大淡入（`popQueueRef`），技能卡则跳过这次展示。
...
- **遮罩淡出 ≠ 演出结束，中间那一两秒要另有一把锁**：遮罩一开始淡出就不再吃指针事件了，
  ...
  对不上号就不放——比如联机时技能卡的兜底定时器先到点放了锁、玩家立刻又打出一张 AI 卡，
  迟到的技能展示时间线跑完时放掉的就会是别人的锁。
- **我方出牌的两条链路都要挂兜底解锁定时器**（`PLAY_LOCK_FALLBACK`）：它们的解锁都要等
  对面把新局面/事件送回来（AI 卡等新局面才有落点可飞，技能卡等 `SKILL_PLAYED` 才亮相），
...
- key 带版本号，结构要改就换下一个版本号：旧数据读不到自动当新号，不写迁移代码，
  老存档整份作废重来。v4 里存的卡 id 全部来自当前卡池（AI 卡 + 技能卡两类）；
  英雄不进牌组也不进收藏，存档里没有英雄这一项。
...
packages/core/
  ...
  src/heroes.ts               英雄数据 + 查表（不进牌组，和卡牌是两张表）
...
- **答题制的对局流程整条打通了**：抛硬币定先手 → 双方轮流出牌（AI 卡上场、技能卡亮相）→
...
- **对局界面已经是真 UI**：底部 `HandFan` 扇形手牌拖进战场（或原地点一下）出牌、
  ...
  点战场小卡也能放大查看。
- **英雄系统的第一版**：目前只有格蕾丝·霍珀一位，双方开局默认都用她（没有选英雄的界面）。
  Debug 在引擎里真的生效（抵消对方本局第一张技能卡，见 3.4），对局左侧栏能看到双方的英雄卡
...
2. **技能卡的效果**——现在打出去只有卡面和亮相动画，不产生任何效果，也不选目标。
   动手时注意：效果必须写在"没被英雄技能抵消"的那条分支里（见 3.4 的 Debug）。
3. **补卡池和题库**——卡池只有 4 张 AI 卡 + 1 张技能卡，题库 5 道占位题
   （也就是一局固定 5 轮），视觉测试那两道的图还没有，题面先用文字描述顶着。
   卡池扩容顺带让赢局抽卡重新生效（`INITIAL_COLLECTION` 现在等于整个卡池，见 3.5）。
4. **卡组选择和选英雄**（client）——现在联机双方都写死用 `STARTER_DECK`，
   英雄也都是默认那一位；`PlayerSetup` 的 `deck` / `hero` 两个口子都已经开好了。
