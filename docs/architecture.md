...
- `SkillCard`（`kind: 'skill'`）打出后直接进弃牌堆。`category` 是卡面规则分类，`effect` 是结算方式，
  `target` 决定交互与合法目标：`'foe-ai'` 选对方未受本轮提示词效果的 AI，
  `'own-ai-interference'` / `'own-ai-restriction'` 分别选己方受到「干扰」/「限制」的 AI，
  `'own-ai-reflectable'` 选己方承受了对方非环境技能的 AI。该填不填、阵营不符或效果类别不符，
  都会被 `COMMAND_REJECTED` 挡回去；没有 `target` 的卡带上 `targetInstanceId` 则直接忽略。
...
`SkillCard.tokenCost` 是已经接入规则引擎的游戏内 Token 费用：目前「复读机」4、
「黑白颠倒」3、「一句话回答」1、「字数封锁」3、「大扫除」3、「玉净瓶」4、
「弹弹弹」3、「金钟罩」7、「保送」3、「防沉迷」1、「算力压缩」2、「模型蒸馏」3、「开源复现」4；
未配置费用的占位技能按 0 处理。**AI 牌费用也已接入扣费**：`AiCard.tokenCost` 是权威来源
（见 `aiModels.ts`），打出 AI 牌按此扣 Token，客户端卡面只从 core 取值展示。
每名玩家默认带着整局 100 Token 开局，余额保存在 `PlayerState.tokens`，合法出牌时立即扣除；
余额不足时整条指令被拒绝，状态保持原样。

几张围绕 Token / 出牌资源的技能会改动 `PlayerState` 上的本轮字段：
「算力压缩」写 `nextAiDiscount`（下一张 AI 牌费用减 2、最低 1，打出 AI 牌即消费，跨轮保留直到用掉）；
「防沉迷」写对手的 `playLimitThisRound`（本轮出牌上限，含 AI 牌和技能牌），配合每次出牌累加的
`playsThisRound` 判断是否超限，两者都在答题结算后清零；「保送」把目标 AI 的
`guaranteedNextRound` 记为 `'safe-pass'`，与李飞飞的保送共用同一字段、同样结算后清除。
引擎眼下仍不限制己方每轮出几张牌（防沉迷是对手施加的限制），
「每轮至多打出一张 AI 牌」这条设计目标尚未写进引擎。

**英雄。** `PlayerState.hero`（`HeroId | null`）是这一方选的英雄牌，
`PlayerState.heroSkillUsed` 记当前技能是否不可再发动。开局由 `PlayerSetup.hero` 指定，
不填仍以格蕾丝·霍珀兜底；选择页会把本机选择存入 `selectedHero.ts`，联机客人通过 `match:ready`
先把英雄报给房主，房主拿到双方配置后才创建开局。测试房也读取本机选择。
"每局一次"的一局就是一个 `GameState` 的生命周期，`createGame` 重新建状态时标志天然回到 false；
李飞飞是每个图片题回合可发动一次，进入下一轮时单独重置。
英雄初始化不消耗随机数，不影响 `createGame` 里那串随机数的顺序（见本节末尾）。

人物名单已经定下 7 位（见机制文档 §4），技能设计好并接入规则后才进 `HEROES`。目前共 4 位：

- 李飞飞（Fei-Fei Li），**再看一眼**：图片题回合选择一个己方 Agent，答错也不会被罚下；每个图片题回合一次。
- 陈丹琦（Danqi Chen），**精准检索**：每局一次，选择一个己方可升级 Agent，本轮使用同系列下一代模型答题。
- 梅拉妮·珀金斯（Melanie Perkins），**化繁为简**：每局一次，选择一个对方可降级 Agent，本轮使用同系列上一代模型答题。
- 格蕾丝·霍珀（Grace Hopper），**Debug**：每局抵消对方打出的第一张技能牌的效果。

前三项主动技能走 `USE_HERO_SKILL`，只能在自己的出牌阶段使用。陈丹琦作用于己方、梅拉妮作用于对方；
版本关系在 `aiModels.ts` 的 `AI_MODEL_UPGRADES` / `AI_MODEL_DOWNGRADES` 显式维护，单版本或已经到链端的 Agent
不是合法目标。临时版本写入 `AiInstance.roundModelOverride`，原始 `cardId` 不变，所以被罚下时回收的仍是原始牌。
李飞飞的保送写入 `AiInstance.guaranteedNextRound`。这两项都在答题结算后清除。
...
技能效果都在这里结算，被抵消时不能留下 `promptEffect`、净化、反弹或护盾状态，
否则会出现"技能被抵消了，规则状态却已经变化"的自相矛盾局面。
...
弃牌堆。`AiInstance` 除了身份三件套（`instanceId` / `cardId` / `owner`），还有本轮可选的
`promptEffect`、`roundModelOverride` 和 `guaranteedNextRound`。`promptEffect` 记录来源技能牌、来源玩家、
分类、要追加到提示词的 `instruction`，以及固定剧本的模拟方式。
同一个 AI 本轮只能挂一条，战场小卡按分类显示「已干扰」或「受限制」；答题结算后统一清除，下一轮可以再选。
`PlayerState.skillShielded` 记录「金钟罩」的本轮保护状态：生效时立即清掉己方已有提示词效果，并阻止后续技能命中，
结算后与提示词效果一起清除。
...
题面含图片时另设 `includesImage: true`，不能用视觉题类别代替这个事实标志。
`src/script.ts` 的 `scriptedAnswers(question, aiUnits)` 是一张写死的
「题目 × AI 牌 → 对错 + 回答原文」静态表，纯函数、确定性；它还会按 `promptEffect.answerMode`
模拟已实现的提示词技能：「复读机」固定输出香蕉，「黑白颠倒」翻转原判断的对错和回答，
「一句话回答」把多句内容合并为一句但保留原判定，「字数封锁」把最终回答限制为最多 3 个 Unicode 字符，
标点也计数；「上下文洪水」「话题漂移」「重复轰炸」往提示词塞无关信息，离线只加「受干扰」前缀、
不改原判定（其中「上下文洪水」用 `foe-all-ai` 覆盖对方全部作答 Agent，无需点选）。
其余四张牌直接操作状态：「大扫除」移除干扰、「玉净瓶」移除限制、
「弹弹弹」把对方非环境提示词效果移到原施放者的第一个合法 AI、「金钟罩」清除并阻止本轮技能效果。
...
眼下正式卡池是 18 张 AI 牌加八张技能牌，`INITIAL_COLLECTION` 暂时也列出这 26 张，因此当前等于整个卡池；
默认 20 张牌组仍是 18 张 AI 加「复读机」「黑白颠倒」，其余六张已实现技能暂不硬塞进默认牌组，
...
- **技能牌亮相**（`SKILL_PLAYED`）：双方都在战场中央把卡面亮一下再进弃牌堆。
  ...
  事件带 `targetInstanceId` 时，亮相之后那张卡不再原地淡出，而是**飞向目标格**
  并在落点播一次命中特效（`playSkillHitFx`：目标格轻抖一下 + 边缘追光闪一圈，
  比上场特效轻一档，不震屏也不扬尘）。敌我两边共用这一段：我方从 `.battle__skill-show` 起飞，
  对方从展示位起飞（见 5.8）。目标格上的「已干扰」或「受限制」角标跟着本轮状态渲染，不靠动画残留。
- **选目标**（客户端自己的一档，没有对应事件）：标了 `target` 的技能牌打出时不直接发指令，
  先按 `target` 让玩家选择对方或己方的合法 AI。**两条路都通，随玩家习惯**，
  分岔点是 `HandFan` 的 `onPlay(id, via)` 那个 `via`（`'drag'` / `'tap'`）：

  - **拖出来**（`'drag'`）：牌一离手（`HandFan` 的 `onDragStateChange` 通知上来），
    对应战场行里每张合法目标就亮一圈会呼吸的**橙色**描边。松手时按**牌自己的中心**
...
具名 AI 的费用角标由 core 的 `AiCard.tokenCost` 提供规则数值，客户端只取值展示、不另存一份；
已实现技能牌的费用直接烘焙在原画中，同样由 core 的 `tokenCost` 提供规则数值。场上的 AI 仍没有会变的数值，
...
一起报给 `onPlay`，因为要选目标的技能牌两条路的后续完全不同（见 5.5）。几个要点：
...
packages/core/
  ...
  src/heroes.ts               英雄牌数据 + 查表（不进牌组，和卡牌是两张表）；
                              名单已定 7 位，当前 4 位已进入正式规则表
                              （七张卡面素材都在 client/public/hero/）
...
packages/client/
  ...
  src/ui/
    ...
    playSummonFx.ts           战场小卡的落点特效，敌我共用一份：卡牌落场（震屏 + 烟尘 + 边缘追光）
                              和技能命中（目标格轻抖 + 同一圈追光，轻一档）
...
- **对局界面已经是真 UI**：底部 `HandFan` 扇形手牌拖进战场（或原地点一下）出牌、
  ...
  点战场小卡也能放大查看；要选目标的技能牌拖出去松手在谁身上就打谁、点一下则进全屏压暗的
  选目标态（两条路合法目标都亮橙圈），命中时技能牌飞到目标格上闪一下，
  那个 AI 在本轮挂着「已干扰」角标，答题结算后清除。
- **英雄牌**：人物名单定了 7 位，李飞飞、陈丹琦、梅拉妮·珀金斯和格蕾丝·霍珀已经进入正式规则表。
  选择页会保存本机选择，联机通过 `match:ready` 交换双方英雄；对局左侧栏可发动主动技能并点选 Agent。
  Debug 自动抵消对方本局第一张技能牌，另外三项技能及临时状态见 3.4。
...
2. **继续补技能牌效果**——「复读机」「黑白颠倒」「一句话回答」「字数封锁」
   「上下文洪水」「话题漂移」「重复轰炸」「大扫除」「玉净瓶」「弹弹弹」「金钟罩」
   「保送」「防沉迷」「算力压缩」「模型蒸馏」「开源复现」以及核电站等环境牌
   已经接入正式卡表、费用、目标交互与状态结算；提示词类还接了固定剧本模拟，
   `promptEffect.instruction` 也保留了将来拼进真实 Prompt 的原始指令。
   其余技能牌目前仍只有图鉴与卡面。新效果必须写在"没被英雄技能抵消"的分支里，
   本轮效果还要在答题结算后清理（见 3.4）。
3. **其余 3 位英雄**——选择页、联机协议和 4 位英雄技能已经接通（见 3.4）；
   米拉·穆拉蒂、阿达·洛芙莱斯、玛格丽特·汉密尔顿仍只展示原画，技能设计并实现后再进 `core/src/heroes.ts`。
4. **补卡池和题库**——卡池是 18 张 AI 牌 + 8 张技能牌，题库 5 道占位题
