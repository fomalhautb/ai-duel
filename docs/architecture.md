...
- `SkillCard`（`kind: 'skill'`）打出后直接进弃牌堆。多数只有卡面和亮相动画；
  ...
  ——对方场上一个本轮还没有 `promptEffect` 的 AI（目前是「复读机」和「黑白颠倒」）。
...
提示词干扰效果（给目标写入 `promptEffect`）就是这么挂上去的，被抵消时一个标记都不留，
...
弃牌堆。`AiInstance` 除了身份三件套（`instanceId` / `cardId` / `owner`）只多一个本轮可选的
`promptEffect`：记录来源技能牌、要追加到提示词的 `instruction`，以及固定剧本的模拟方式。
同一个 AI 本轮只能挂一条，战场小卡同步显示「已干扰」角标；答题结算后统一清除，下一轮可以再选。
...
- **技能牌亮相**（`SKILL_PLAYED`）：双方都在战场中央把卡面亮一下再进弃牌堆。
  ...
  对方从展示位起飞（见 5.8）。目标格上的「已干扰」角标跟着本轮 `promptEffect` 渲染，不靠动画残留。
...
2. **继续补技能牌效果**——「复读机」和「黑白颠倒」已经接入正式卡表、选目标交互、
   命中动画与固定剧本模拟；`promptEffect.instruction` 也保留了将来拼进真实 Prompt 的原始指令。
