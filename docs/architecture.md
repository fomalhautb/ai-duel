...
- **开局抛硬币**（`GAME_STARTED`）：全屏挂一枚 CSS 圆片，`rotationY` 连转四圈之后落到
  「你先出牌」或「对方先出牌」那一面，停一下再整层淡出。转动补间是这里自己写的
...
- **遮罩淡出 ≠ 演出结束，中间那一两秒要另有一把锁**：遮罩一开始淡出就不再吃指针事件了，
  ...
  对不上号就不放——比如联机时技能牌的兜底定时器先到点放了锁、玩家立刻又打出一张 AI 卡，
  迟到的技能展示时间线跑完时放掉的就会是别人的锁。
- **我方出牌的两条链路都要挂兜底解锁定时器**（`PLAY_LOCK_FALLBACK`）：它们的解锁都要等
  对面把新局面/事件送回来（AI 卡等新局面才有落点可飞，技能牌等 `SKILL_PLAYED` 才亮相），
