...
- **只有悬停才出现的东西必须另有出路**：卡面倾斜和高光在触屏上整个关掉（`ui/cardTilt.ts`）；
  手牌和卡池右上角的问号都从"移入翻面"换成"点一下翻一次"；那个问号连同牌组格子上的「×」
  在粗指针下常驻显示——它们原本只在 hover 时才露出来，触屏永远等不到，
...
**怎么计分。** 一轮最多分出 1 分，判据分两层：先比双方本轮**答对的 AI 数**
（数的是提交上来的 `AnswerResult` 里 `correct` 为真的条数，不是罚完之后场上还剩几个
——被「保送」的单位答错也留场，拿场上数量当答对数会把它算成答对），多的一方 +1；
一样多再比**本轮花掉的 Token**，少的一方 +1——同样的正确数用更省的牌拿到，这一分归他；
两条都打平就双方各 +1：谁也没占到便宜，不该有人被扣着不给分。

算完发一条 `ROUND_SCORED`，四个数组都按座位号排：`gains` 是本轮各自拿到的分
（只可能是 0 或 1，打平就是 `[1, 1]`），`scores` 是加完之后的总分，
`correct` / `spent` 是上面那两层判据本身。判据跟着一起发，界面才讲得清
"凭什么这一分给了谁"。
...
`effectivePlayCost(state, playerId, card)`：「核电站」每打出一张就让双方后续的牌便宜 1 点
（计数在 `GameState.costReduction`，可叠加、最低减到 1 点、进下一轮清零），
而打出过「金钟罩」的那一方连这份便宜也不享受（那张牌是字面全挡的口径，见下）。
这个函数是 core 导出的：客户端的"打不起就变灰"必须和引擎的扣费校验用同一个数，
各算一遍就会出现"看着能打、点下去说 Token 不够"。
`tokens -=` 和 `roundTokenSpent +=` 记的也都是这个实际费用。
...
统一在 `confirmRound` 里真的进下一轮那一步清掉：`GameState.costReduction` 清零、
双方的 `PlayerState.shielded` 删掉、场上每个单位的 `interference` / `safePassed` 删掉。
清除点定在这里而不是提交答题结果时，是因为结算界面还要照着这些标记演一遍
"这个被干扰了 / 这个是被保送留下的"。

「金钟罩」的口径是用户拍板的**字面全挡**：`shielded` 期间除那张金钟罩自身外，
任何技能牌（无论谁打的）都不作用于这一方和他的 Agent——对方的干扰牌选不中他的 AI
（选目标那一步直接拒绝）、他自己也打不出「玉净瓶」「保送」「模型蒸馏」这类只作用于自己的牌、
群体牌结算时跳过他的场面、核电站的减费他也不享受。
对己方有利的效果一起挡是有意的，卡面就是这么写的。
金钟罩自己是唯一的例外，否则第一张就会把自己挡住、这张牌永远打不出去；
第二张则被"本轮已经有金钟罩了"拒掉。
英雄技能不是技能牌，格蕾丝·霍珀的 Debug 照常能抵消金钟罩本身。

另有一个 `roundTokenSpent` 记本轮已经花掉多少，出牌时和 `tokens` 一起改。
不拿 `tokenMax - tokens` 现算，是因为进下一轮时 `tokens` 会被补满、`tokenMax` 还要涨，
差值当场就没了；而计分的第二判据（消耗少的一方拿分）和结算界面上那行「本轮消耗」
都要在那之前拿到它。清零也因此排在真正进下一轮那一步，不在提交答题结果时。

眼下费用是唯一一处限制每轮出几张牌的东西。机制文档 V0.2 §7.2 里"每轮至多打出一张 AI 牌"
那条设计目标仍然没有写进引擎——它和 Token 是两套彼此重叠的节流，真要加之前先确认还需不需要。
...
答成什么样交给模型自己。
在那之前，`scriptedAnswers` 按这两句话的**等效结果**改写剧本查出来的回答：
复读机一律 `{ correct: false, answer: '香蕉' }`，黑白颠倒把 `correct` 取反
（翻成答对就报标准答案，翻成答错就在原答案前加个否定）。
这一层是临时的，真实 API 接上就整个去掉——引擎那边只认 `AnswerResult`，一行都不用改。
...
- **技能牌亮相**（`SKILL_PLAYED`）：双方都在战场中央把卡面亮一下再进弃牌堆。
  ...
  格子上的本轮标记（干扰按种类分成「复读中」/「已颠倒」、保送是「保送」）跟着快照渲染，
  不靠动画残留；两种标记可以同时挂，所以摞成一列（`.battle__tile-marks`）。
  被金钟罩罩着的那一方在侧栏自己的面板上挂一枚「金钟罩」（`.battle__shield-mark`）——
  它罩的是整个人，场上一个 AI 都没有时也照样生效，挂在卡上就没地方挂了。
...
- **进化**（`AI_TRANSFORMED`，鸡犬升天）：还是同一个单位换了卡面身份，换图这件事 React
  ...
  在事件回调里当场演的话，闪的是即将被换掉的旧卡面。演的是边缘追光扫一圈 + 卡弹一下
  （`playEvolveFx`）。
...
- **答题制的对局流程整条打通了**：抛硬币定先手 → 双方轮流出牌（AI 牌上场、技能牌亮相）→
  全场 AI 答题、答错的罚下 → 结算这一轮谁拿分 → 双方各确认一次才进下一轮 →
  打满题库轮数后按总分分胜负，含平局。
  计分是**每轮最多 1 分**：先比答对的 AI 数，一样多比本轮 Token 消耗谁少，还一样就各 +1（见 3.4）。
...
- **对局界面已经是真 UI**：底部 `HandFan` 扇形手牌拖进战场（或原地点一下）出牌、
  ...
  右侧栏是一块「下一题：<类别>」的繁复牌匾加一排 Token 四芒星（亮的是还剩的、灰的是花掉的，
  永远单列、点数多了只压间距；「模型蒸馏」换来的 Token 可以超过本轮上限，超出的那几颗换成青色），
  比分在顶栏；对手出的牌会被强制展示在屏幕中央（见 5.8），
  点战场小卡也能放大查看；要选目标的技能牌拖出去松手在谁身上就打谁、点一下则进全屏压暗的
  选目标态（两条路合法目标都亮橙圈，四档目标的口径见 5.5），命中时技能牌飞到目标格上闪一下，
  那个 AI 从此挂着对应的角标（「复读中」/「已颠倒」/「保送」）。
  「核电站」开着时战场左上角常驻一条「本轮出牌费用 -N」。
...
- **Token 资源**：第 1 轮上限 4 点，每答完一题补满并 +2，省下的不跨轮累积。
  ...
  `effectivePlayCost`），不够就被引擎拒掉；手牌里买不起的那几张会压暗、拖不动，
  点一下弹「Token 不够」——判据走的正是 core 导出的那个函数（`HandFan` 的 `playCostOf`），
