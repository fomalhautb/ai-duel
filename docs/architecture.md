...
- **强制展示**（对手出牌，不可打断）：收到对手的 `MODEL_DEPLOYED` / `PROMPT_RESOLVED`，
  ...
  这条链路（展示卡、呼吸浮动、遮罩节点）都在 `MatchStage` 自己手里。
- **放大查看**（玩家点战场小卡，敌我两行一视同仁）：格子留在原地隐藏占位
  （`.battle__tile--held`），展示层多渲染一份放大的副本；点遮罩（也就是点屏幕任意处）
  再飞回原格。整段不翻面，时长和层级全部复用上面那套，两条链路的手感才一致。
  这条链路整个收在 `ui/CardZoomOverlay.tsx` 里（展示卡、浮动、`.reveal-*` 的时长缓动常量都归它），
  `/deck` 点开一张卡走的是同一个组件。

两条链路在 `MatchStage` 里**严格互斥**：`revealBusyRef` 挡住展示期间再开查看，
反过来对手出牌时直接 `zoomRef.current.abort()` 把查看掐掉（等不了飞回那 0.6 秒）。
互斥之外只剩一条硬约束：**两边必须补间同一个遮罩节点**——遮罩由 `MatchStage` 渲染，
ref 通过 `veilRef` 交给 `CardZoomOverlay`，淡入淡出靠 `overwrite: 'auto'` 互相接管。
各渲染一块遮罩的话，交替时两块会同时半透明可见（压暗和模糊翻倍），
被抢占的那块更是没人再管，会永远停在全黑上吃掉所有点击。
理由和边界写在 `CardZoomOverlayProps.veilRef` 的注释里，动这块之前先看那里。
只有一条链路会动遮罩的页面（`/deck`）不传 `veilRef`，用组件自带的那块就行。

几个绕不开的点：

- **事件回调在 React 提交新局面之前同步送达**（这也正是"那张牌此刻还在对手手牌 DOM 里"
  的原因），所以"是不是正在展示"只能读 ref 不能读 state（`MatchStage` 的 `revealBusyRef`、
  `CardZoomOverlay` 里在渲染期就同步赋好值的 `targetRef`），同 `seatRef`。
