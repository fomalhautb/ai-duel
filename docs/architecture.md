...
- **只有悬停才出现的东西必须另有出路**：卡面倾斜和高光在触屏上整个关掉（`ui/cardTilt.ts`）；
  手牌和卡池右上角的问号（只有能翻面的牌有，见 5.7）都从"移入翻面"换成"点一下翻一次"；
...
它也不等于"结算后场上还剩人"，被「保送」的单位答错也留在场上。
...
（只可能是 0 或 1，同对同错且消耗相同时是 `[1, 1]`），`scores` 是加完之后的总分，
`correct`（布尔，团队口径）/ `spent` / `verdict` 是上面那三条判据本身。
...
眼下卡池是 18 张 AI 牌加 10 张已开放的技能牌（技能牌共 24 张，另外 14 张是「即将上线」，
不进卡池，见 `skillCards.ts` 的 `OPEN_SKILL_CARD_IDS`；开放的那 10 张正好就是效果已经接进
引擎的 10 张，两份名单是同一批），
...
**问号只长在翻过去有东西看的牌上。** AI 牌的背面是全套通用的那张美术图（`AI_CARD_BACK_ART`），
翻过去一个字都读不到，所以对局手牌和卡组页的卡池卡 / 迷你卡都不给 AI 牌渲染问号，
连背面整层一起省掉（`HandFan` 和 `DeckScreen` 里各有一个 `flippable`）。
卡池里「即将上线」的技能牌同样不给（那一面只是一段还没生效的效果说明，
热区留着还会把"点卡面 = 弹一句即将上线"那一下吃掉），这一条只写在 `DeckScreen` 的 `flippable` 里。
AI 牌的那张卡背只剩两处还在画：卡组页放大查看时右边那张伴随的背面大卡，
以及对局里对手打出的牌飞到屏幕中央途中翻正的那一下。
...
4. **补卡池和题库**——卡池是 18 张 AI 牌 + 10 张已开放的技能牌（另 14 张技能牌「即将上线」），
