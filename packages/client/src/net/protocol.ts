 ...
 * 所以协议改了服务端不用动。
 *
 * 方向是固定的：`match:start` 和 `match:sync` 只有房主发，`match:ready` 和 `match:command` 只有客人发。
 */

import type { Command, GameEvent, GameState, HeroId, PlayerId } from '@ai-duel/core'

export type RelayMessage =
  /** 客人进房后先报自己的英雄，房主收到后才创建完整开局。 */
  | { type: 'match:ready'; hero: HeroId }
  /**
