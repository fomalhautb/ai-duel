...
export const TUTORIAL_CARDS = {
   ...
   *
   * GPT-2 是教学牌组里唯一一张**卡池外**的牌：OpenRouter 上调不到 GPT-2，它在牌组页是灰的、
   * 玩家自己拼不出这副牌（见 core 的 UNAVAILABLE_AI_CARD_IDS）。这里仍然用它，是因为
   * 全卡池只有它是 1 费，而教学局的答案是预设的（见 TUTORIAL_ANSWERS）、根本不调模型。
   * 代价是玩家学完回到牌组页会发现刚用过的这张选不了。要消掉这点不一致，就得改教学设计——
   * 比如第 2 轮干脆不放行增派 AI（玩家最多花 4 < 6，不等式照样成立），那一步的文案跟着改。
   */
 ...
 *
 * 挑的全是卡池里的牌（文心一言那种"调不到模型"的不放）：第 3 轮玩家可以自由出牌，
