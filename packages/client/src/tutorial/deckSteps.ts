 ...
 *
 * 挑的都是教学对战里刚用过的牌——第 1 轮派出去的 GPT-3.5、第 2 轮可以增派的豆包、
 * 第 2 轮那张「必须回答」。玩家对它们有印象，"加入牌组"才不是在选三张陌生卡。
 */
export const TUTORIAL_DECK_PICKS: readonly CardId[] = ['gpt-3-5', 'doubao', 'skill-must-answer']

/**
 * 进组牌页时先替玩家填好的那 17 张（规格 §12：计数从 `17 / 20` 起步）。
 *
 * 取法是"卡池里除掉上面那三张之外的前 17 张"，于是天然满足两条约束：
 * 三张待加的牌一份都还没占（每张卡最多 2 份的规则怎么都碰不到），
 * 而 17 + 3 正好是 DECK_SIZE。卡池现在正好 20 张，所以最后拼出来的是一套"每张一份"的牌组。
 *
 * 卡池扩容后这里仍然只取前 17 张，不用改；缩到 20 张以下才会不够填，由测试守着。
 */
