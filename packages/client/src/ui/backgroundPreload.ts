 ...
 *
 * 为什么要做：每个界面都有自己的加载闸门（首页、英雄页、对局页），闸门等的是"进这一页要用的图"。
 * 玩家从首页一路走到对局，等于要在三个地方分别停下来等一次；而对局用到的那十几 MB 原画，
 ...
 * 玩家的实际路径是首页 → 房间 → 对局，所以对局的背景和硬币最先要，
 * 卡牌原画紧随其后（对局开始才需要，但有 18 张近 10MB，是最大的一块），
