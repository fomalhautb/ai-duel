 ...
 *
 * 这批牌进了卡池、玩家能选进牌组、能真的打出来，但除了「复读机」和「黑白颠倒」效果都没实装。
 * 下面几条守的就是"没实装"这件事在数据上不出岔子：一旦有人给别的卡也填了 target，
 * 引擎就会要求玩家选目标，而选完之后什么都不会发生。
 */
describe('技能卡', () => {
  const cards = Object.values(SKILL_DESIGN_CARDS)
  /** 接进引擎的两张：它们要选目标，也因此不写 plannedEffect（见 skillCards.ts）。 */
  const IMPLEMENTED_IDS = ['fixed-answer', 'black-white-reversal']
  ...
  it('没实装的那些都带设计效果全文，客户端卡背才有话可印', () => {
    expect(unimplemented).toHaveLength(22)
    for (const card of unimplemented) {
      expect(card.plannedEffect).toBeTruthy()
    }
  })

  it('只有那两张干扰牌选目标：其余效果没实装，选了目标也什么都不会发生', () => {
    expect(implemented).toHaveLength(2)
    expect(implemented.map((card) => card.target)).toEqual(['foe-ai', 'foe-ai'])
    // 实装了的不写 plannedEffect：带上它卡背会印"还没实装"，而它们确实会改场上的状态。
    expect(implemented.map((card) => card.plannedEffect)).toEqual([undefined, undefined])
    for (const card of unimplemented) {
